...
- [x] **Step 1: Write failing model tests** asserting one Camera can own exactly one current connection, RTSP config fields round-trip, and default revision/status are `1` / `unverified`.
- [x] **Step 2: Run** `cd backend && uv run pytest tests/test_camera_connection_model.py -v`; expect failure because the models do not exist.
- [x] **Step 3: Implement minimal ORM models and exports.**
- [x] **Step 4: Re-run the focused test and then** `cd backend && uv run pytest tests/test_camera_connection_model.py tests/test_camera_connection_type.py -v`.
- [x] **Step 5: Commit** `feat: add camera current connection models`.
...
- [x] **Step 1: Write failing migration tests** using a temporary SQLite database upgraded through `20260916_0019`, insert representative legacy Camera rows, upgrade to `20260916_0020`, and assert IDs/connection fields/password ciphertext are unchanged.
- [x] **Step 2: Add a failing test** inserting a legacy non-RTSP Camera and assert upgrade to `0020` aborts with its Camera ID.
- [x] **Step 3: Run** `cd backend && uv run pytest tests/test_camera_connection_migration.py -v`; expect failure because revision `0020` is absent.
- [x] **Step 4: Implement migration** with schema creation, strict preflight guard, RTSP backfill, uniqueness constraints, and downgrade that drops only new connection tables.
- [x] **Step 5: Re-run focused migration tests.**
- [x] **Step 6: Commit** `feat: migrate legacy RTSP cameras to current connections`.
...
- [x] **Step 1: Write failing integration tests** that create Camera + Recording / MotionEvent / HealthSample and assert deleting Camera raises an integrity failure while historical rows remain.
- [x] **Step 2: Run** `cd backend && uv run pytest tests/test_camera_history_fk.py -v`; verify current cascade behavior makes the test fail.
- [x] **Step 3: Implement SQLite-safe Alembic table rebuilds/batch alters and ORM relationship changes.**
- [x] **Step 4: Re-run focused tests and migration upgrade/downgrade smoke tests.**
- [x] **Step 5: Commit** `fix: protect camera history from cascade deletion`.
...
- [x] **Step 1: Write failing tests** proving audit/lifecycle events do not block deletion and explicitly marked business events do.
- [x] **Step 2: Run focused test; expect missing field/signature failure.**
- [x] **Step 3: Add model/migration/service parameter and conservative backfill for existing rows (`recording_id IS NOT NULL` or monitoring-event categories become blocking; audit/lifecycle stay non-blocking).**
- [x] **Step 4: Update existing event producers only where business-history semantics are clear.**
- [x] **Step 5: Run event-related test suite.**
- [x] **Step 6: Commit** `feat: classify camera history events for deletion`.
...
- [x] **Step 1: Write failing service/API tests** for empty Camera deletion, recorded Camera refusal, health-only refusal, and non-blocking audit events not preventing deletion.
- [x] **Step 2: Run focused tests; verify current DELETE behavior fails them.**
- [x] **Step 3: Implement deletion-impact schema/service and endpoint.**
- [x] **Step 4: Refactor DELETE to call the impact service before any destructive ORM operation.**
- [x] **Step 5: Run** `cd backend && uv run pytest tests/test_camera_deletion.py tests/test_camera_history_fk.py tests/test_event_delete_blocking.py -v`.
- [x] **Step 6: Commit** `feat: guard camera deletion with history impact`.
...
- [x] **Step 1: Run backend suite:** `cd backend && uv run pytest -q`.
- [x] **Step 2: Run lint:** `cd backend && uv run ruff check app tests migrations`.
- [x] **Step 3: Run Alembic head smoke upgrade on a fresh SQLite database and an upgrade from revision `20260916_0019` populated with RTSP fixtures.**
- [x] **Step 4: Verify migration invariants:** same Camera count/IDs; same Recording/MotionEvent/HealthSample counts and `camera_id`; identical password ciphertext; one current connection per Camera.
- [x] **Step 5: Update the design/migration checklist with completed Phase 1 items.**
- [x] **Step 6: Commit verification/doc fixes if needed and open a PR for Phase 1 review.**
