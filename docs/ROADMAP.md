...
- [x] 摄像头详情增加 ONVIF 能力区：设备身份、Media Profiles、主 / 子角色及 Media / Events / PTZ 服务地址标记；事件订阅与 PTZ 控制仍单独待办

### B. ONVIF 与设备发现

- [x] ONVIF 基础客户端与认证
- [x] 局域网设备发现（明确开启后执行，不默认后台扫描）：ONVIF WS-Discovery；Manual RTSP 仅扫描默认路由 /24 的 TCP 554
- [ ] 自动读取 manufacturer / model / firmware / serial / MAC（manufacturer / model / firmware / serial / hardware id 已读取并展示；MAC 尚未实现）
