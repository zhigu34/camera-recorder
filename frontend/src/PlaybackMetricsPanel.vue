...
const emit = defineEmits<{ (event: 'open-playback'): void }>()
...
const compatibilityProblems = computed(() =>
  (metrics.value?.client.compatibility || []).filter((item) => (item.success_rate ?? 100) < 98).length,
)
...
async function loadMetrics() {
  ...
  try {
    ...
    // Keep the parent page usable when playback metrics are temporarily unavailable.
  } finally {
    loading.value = false
  }
}
...
<template>
  <Teleport to=".page-shell" :disabled="compact" defer>
    <div id="playback-compatibility" class="playback-health" :class="{ compact, embedded: !compact }" v-loading="loading && !metrics">
      <el-card shadow="never">
        <template #header>
          <div class="panel-head">
            <div>
              <strong>{{ compact ? 'Web 回放健康' : 'Web 回放质量 / 浏览器兼容性' }}</strong>
              <div class="subtitle">{{ compact ? '播放成功率、首帧和浏览器异常摘要' : '真实播放器事件 · 内存统计 · 服务重启后清空' }}</div>
            </div>
            <div class="panel-actions">
              <el-button v-if="compact" size="small" type="primary" plain @click="emit('open-playback')">查看回放详情</el-button>
              <el-button v-else size="small" :loading="loading" @click="loadMetrics">刷新</el-button>
            </div>
          </div>
        </template>
        ...
        <template v-if="metrics">
          <template v-if="compact">
            <div class="compact-metrics">
              <div><span>首帧成功率</span><strong :class="`rate-${rateType(startupSuccessRate)}`">{{ formatRate(startupSuccessRate) }}</strong></div>
              <div><span>首帧 P95</span><strong>{{ formatMs(metrics.client.events.first_frame.p95_ms) }}</strong></div>
              <div><span>启动失败</span><strong :class="{ danger: metrics.client.events.startup_error.count > 0 }">{{ metrics.client.events.startup_error.count }}</strong></div>
              <div><span>兼容异常组</span><strong :class="{ danger: compatibilityProblems > 0 }">{{ compatibilityProblems }}</strong></div>
            </div>
            <div class="compact-hint">详细的 Chrome / Edge / Safari / Firefox、H.264 / HEVC、原片 / Proxy 兼容矩阵已归入录像回放页。</div>
          </template>
...
<style scoped>
.playback-health{max-width:1500px;margin:16px auto 24px;padding:0 24px;box-sizing:border-box}.playback-health.embedded{width:100%;max-width:none;margin:0;padding:0}.panel-head{display:flex;justify-content:space-between;align-items:center;gap:12px}.panel-actions{display:flex;align-items:center;gap:8px}.subtitle{margin-top:4px;color:var(--nvr-muted);font-size:12px}.metric-row{margin-bottom:14px}.metric-box{min-height:72px;border:1px solid var(--nvr-border);border-radius:7px;background:var(--nvr-surface-2);display:flex;flex-direction:column;justify-content:center;align-items:center;gap:5px}.metric-box strong{font-size:21px;color:var(--nvr-text)}.metric-box span{font-size:12px;color:var(--nvr-muted)}.backend-strip{display:flex;flex-wrap:wrap;gap:8px 18px;padding:10px 12px;margin-bottom:14px;background:var(--nvr-surface-2);border-radius:6px;color:var(--nvr-muted);font-size:12px}.backend-strip b{color:var(--nvr-text)}.compact-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1px;border:1px solid var(--nvr-border);border-radius:8px;background:var(--nvr-border);overflow:hidden}.compact-metrics>div{padding:12px 14px;background:var(--nvr-surface-2);display:flex;flex-direction:column;gap:5px}.compact-metrics span{color:var(--nvr-muted);font-size:10px}.compact-metrics strong{color:var(--nvr-text);font-size:18px}.compact-metrics strong.danger,.compact-metrics strong.rate-danger{color:var(--nvr-red)}.compact-metrics strong.rate-warning{color:var(--nvr-yellow)}.compact-metrics strong.rate-success{color:var(--nvr-green)}.compact-hint{margin-top:10px;color:var(--nvr-muted);font-size:10px;line-height:1.6}@media(max-width:720px){.playback-health{padding:0 14px}.playback-health.embedded{padding:0}.panel-head{align-items:flex-start}.compact-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}
</style>
