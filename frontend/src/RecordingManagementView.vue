...
const pendingArchiveCount = computed(() => recordings.value.filter((row) => ['pending', 'uploading', 'retry_wait', 'failed'].includes(row.upload_status)).length)
...
const filteredRecordings = computed(() => {
  ...
  return recordings.value.filter((row) => {
    ...
    const text = `${name} ${camera?.ip || ''} ${filename} ${row.mp4_path}`.toLowerCase()
...
function cameraName(cameraId: number) { return cameraMap.value.get(cameraId)?.name || `摄像头 #${cameraId}` }
...
function clearFilters() {
  searchText.value = ''; cameraFilter.value = null; uploadFilter.value = ''; healthFilter.value = ''; storageFilter.value = ''; dateRange.value = null
}
function openDetail(row: Recording) { activeRecording.value = row; detailVisible.value = true }
async function load() {
  loading.value = true
  try {
    const [recordingRes, cameraRes, statusRes] = await Promise.all([
...
<template>
  <div class="recording-page" v-loading="loading">
    <section class="summary-grid">
      <article class="summary-card"><span class="summary-icon"><DataLine /></span><div><small>已加载录像</small><strong>{{ recordings.length }}</strong><em>{{ formatBytes(totalSize) }}</em></div></article>
      <article class="summary-card"><span class="summary-icon"><Cloudy /></span><div><small>已归档</small><strong>{{ archivedCount }}</strong><em>{{ uploadStatus?.active ? '自动归档运行中' : '自动归档未运行' }}</em></div></article>
      <article class="summary-card"><span class="summary-icon"><Cloudy /></span><div><small>待处理归档</small><strong>{{ pendingArchiveCount }}</strong><em>详细任务统一在上传管理处理</em></div></article>
      <article class="summary-card" :class="{ danger: abnormalCount > 0 }"><span class="summary-icon"><WarningFilled /></span><div><small>异常录像</small><strong>{{ abnormalCount }}</strong><em>健康检查或时间戳告警</em></div></article>
    </section>

    <section class="panel filter-panel">
      <div class="filter-row">
        <el-input v-model="searchText" clearable placeholder="搜索摄像头、文件名或本地路径" class="search-box"><template #prefix><Search /></template></el-input>
      ...
      <div class="filter-actions"><span>当前加载 {{ filteredRecordings.length }} / {{ recordings.length }} 条</span><el-button link @click="clearFilters">清除筛选</el-button><el-button @click="load">刷新</el-button><el-button type="primary" plain @click="emit('open-uploads')">上传管理</el-button></div>
    </section>
