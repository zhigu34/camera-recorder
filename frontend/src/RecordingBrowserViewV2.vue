...
async function fetchDay(date: string): Promise<BrowserResult> {
  ...
  const response = await axios.get<BrowserResult>('/api/recordings/browser', {
    params: { camera_id: selectedCamera.value, date },
  })
...
async function loadRecordings() {
  ...
  loading.value = true
  try {
    data.value = await fetchDay(selectedDate.value)
    if (activeRecording.value) {
      const refreshed = data.value.items.find((item) => item.id === activeRecording.value?.id)
      if (refreshed) activeRecording.value = refreshed
    }
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.detail || error?.message || '录像加载失败')
  } finally {
    loading.value = false
  }
}
...
async function selectDay(date: string) {
  const previousMonth = calendarMonth.value
  selectedDate.value = date
  calendarMonth.value = date.slice(0, 7)
  if (calendarMonth.value !== previousMonth) await Promise.all([loadRecordings(), loadCalendar()])
...
async function selectCalendarDay(cell: CalendarCell) {
  if (cell.date) await selectDay(cell.date)
}
...
async function jumpToLatest() {
  ...
  const date = recordingDate(latestRecording.value.started_at) || todayString()
  selectedDate.value = date
  calendarMonth.value = date.slice(0, 7)
  await Promise.all([loadRecordings(), loadCalendar()])
}
...
async function play(item: RecordingItem) {
  ...
  if (!item.playback.original_available && item.playback.state === 'ready') {
    playbackMode.value = 'proxy'
    playbackNotice.value = '原片已清理，直接播放已有 H.264 Proxy 缓存'
    videoSrc.value = streamUrl(item.id, 'proxy')
    await nextTick()
    return
  }
  ...
  if (isH264(item.video_codec) || isHevc(item.video_codec)) {
    ...
    if (isHevc(item.video_codec)) {
      ...
      playbackNotice.value = cloudStream
        ...
        : 'HEVC 原片优先直放，失败后自动边转 H.264'
    } else {
      playbackNotice.value = cloudStream
        ? 'OpenList 云端 H.264 原片直放；优先 302 直链，必要时 Range 透传'
        : 'H.264 原片直放，不转码'
    }
    videoSrc.value = streamUrl(item.id, 'original')
    await nextTick()
...
async function navigateAdjacent(direction: AdjacentDirection, automatic = false): Promise<boolean> {
  ...
  if (localTarget) {
    await play(localTarget)
    return true
  }

  navigationLoading.value = true
  try {
    const response = await axios.get<AdjacentResult>(`/api/recordings/${activeRecording.value.id}/adjacent`, {
      params: { direction },
    })
    const target = response.data.item
    if (!target) {
      if (!automatic) ElMessage.info(direction === 'next' ? '已经是该摄像头最后一段可播放录像' : '已经是该摄像头第一段可播放录像')
      return false
    }
    ...
    if (targetDate && targetDate !== selectedDate.value) {
      ...
      loading.value = true
      try {
        data.value = await fetchDay(targetDate)
      } finally {
        loading.value = false
      }
      if (calendarMonth.value !== previousMonth) await loadCalendar()
      playableTarget = data.value?.items.find((item) => item.id === target.id) || target
      ElMessage.info(`${direction === 'next' ? '下一段' : '上一段'}已跨日到 ${targetDate}`)
    }

    await play(playableTarget)
    return true
  } catch (error: any) {
    ElMessage.error(error?.response?.data?.detail || error?.message || '相邻录像定位失败')
    return false
  } finally {
...
async function playPrevious() {
  await navigateAdjacent('previous')
}

async function playNext() {
  await navigateAdjacent('next')
}

function handleVideoCanPlay() {
  preparing.value = false
  const sourceKind = activeRecording.value?.playback.source_kind
  const sourcePrefix = sourceKind === 'openlist_stream' ? 'OpenList 云端流 · ' : sourceKind === 'cloud_cache' ? '云端缓存 · ' : ''
...
async function handleVideoEnded() {
  if (!autoAdvance.value) {
    preparing.value = false
    playbackNotice.value = '当前片段播放完成，自动续播已关闭'
    return
  }
  const moved = await navigateAdjacent('next', true)
  if (!moved) {
    preparing.value = false
    playbackNotice.value = '已播放到该摄像头最后一个可播放片段'
  }
}
...
<template>
  <div class="page-shell">
    <div class="page-head">
      <div>
        <h2>录像浏览</h2>
        <p>连续回看 · 跨自然日自动续播 · 录像日历 · OpenList 直链 / Range 云端回放</p>
      </div>
    ...
    <el-card shadow="never" class="section-gap" v-loading="loading">
      ...
      <div class="timeline">
        <div v-for="hour in [3, 6, 9, 12, 15, 18, 21]" :key="hour" class="grid-line" :style="{ left: `${hour / 24 * 100}%` }" />
        ...
        <button
          ...
          :title="`${localClock(item.started_at)} · ${formatDuration(item.duration)} · ${storageLabel(item)}`"
          @click="play(item)"
        />
      </div>
      <div class="legend">
        ...
        <span><i class="dot cloud-dot" />仅云端</span>
    ...
    <el-card shadow="never" class="section-gap">
      ...
      <el-table :data="recordings" stripe empty-text="当前摄像头在所选日期暂无录像" :row-class-name="recordingRowClassName">
        ...
        <el-table-column label="位置" width="120"><template #default="{ row }"><el-tag :type="isCloudOnly(row) ? 'warning' : row.playback.original_available ? 'success' : 'info'">{{ storageLabel(row) }}</el-tag></template></el-table-column>
    ...
    <el-dialog v-model="playerVisible" width="min(1000px, 92vw)" destroy-on-close title="录像回放" @closed="closePlayer">
      <div v-if="activeRecording" class="player-meta">
        <strong>{{ recordingDate(activeRecording.started_at) }} {{ localClock(activeRecording.started_at) }}</strong>
        ...
        <span v-if="activePlayablePosition">当天 {{ activePlayablePosition }} / {{ playableRecordings.length }}</span>
      ...
      <div class="player-controls">
        <div class="navigation-controls">
          <el-button :loading="navigationLoading" :disabled="preparing || !activeRecording" @click="playPrevious">上一段</el-button>
          <el-button :loading="navigationLoading" :disabled="preparing || !activeRecording" @click="playNext">下一段</el-button>
        </div>
        <label class="auto-advance-control"><span>播放结束自动续播（可跨日）</span><el-switch v-model="autoAdvance" /></label>
      </div>

      <div v-if="playbackNotice" class="playback-notice">{{ playbackNotice }}</div>
      <div class="player-box" v-loading="preparing || navigationLoading" :element-loading-text="playerLoadingText">
        <video
          v-if="videoSrc"
          :src="videoSrc"
          controls
        ...
        <div v-else-if="preparing || navigationLoading" class="prepare-note">正在准备播放源…</div>
      </div>
    </el-dialog>
  </div>
</template>
...
.gap-marker { position: absolute; top: 0; bottom: 0; min-width: 2px; background: repeating-linear-gradient(135deg, rgba(230,162,60,.32) 0, rgba(230,162,60,.32) 5px, rgba(230,162,60,.08) 5px, rgba(230,162,60,.08) 10px); z-index: 1; }
.segment { position: absolute; top: 15px; height: 44px; border: 0; border-radius: 4px; background: #67c23a; cursor: pointer; min-width: 3px; opacity: .92; z-index: 2; }
...
.normal { background: #67c23a; }.warning-dot { background: #e6a23c; }.bad { background: #f56c6c; }.cloud-dot { background: #7c3aed; }.deleted-dot { background: #c0c4cc; }.active-dot { background: #409eff; }.gap-dot { background: #e6a23c; }
