  ...
  <section class="motion-panel" v-loading="loading">
    ...
    <template v-if="config">
      ...
      <div class="setting-grid">
        <label><span>灵敏度</span><el-select v-model="config.sensitivity" @change="saveSettings"><el-option label="低" value="low" /><el-option label="中" value="medium" /><el-option label="高" value="high" /></el-select></label>
        <label><span>最短移动时间</span><el-select v-model="config.min_duration_ms" @change="saveSettings"><el-option label="0.5 秒" :value="500" /><el-option label="0.8 秒" :value="800" /><el-option label="1.5 秒" :value="1500" /><el-option label="3 秒" :value="3000" /></el-select></label>
        <label><span>事件合并间隔</span><el-select v-model="config.merge_gap_ms" @change="saveSettings"><el-option label="1 秒" :value="1000" /><el-option label="3 秒" :value="3000" /><el-option label="5 秒" :value="5000" /><el-option label="10 秒" :value="10000" /></el-select></label>
        <label><span>分析帧率</span><el-select v-model="config.analysis_fps" @change="saveSettings"><el-option label="3 FPS" :value="3" /><el-option label="5 FPS" :value="5" /><el-option label="8 FPS" :value="8" /></el-select></label>
      </div>

      <div class="zone-section">
        <div class="zone-head"><div><strong>检测区域</strong><span>{{ config.zones.length ? '仅区域内运动会生成事件' : '未配置区域时检测整个画面' }}</span></div><el-button size="small" :icon="Plus" @click="openCreateZone">添加区域</el-button></div>
