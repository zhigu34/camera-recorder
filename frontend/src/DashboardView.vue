...
const loading = computed(() => runtimeLoading.value && !summary.value && activityLoading.value)
...
const latestActivityByCamera = computed(() => {
  const rows = new Map<number, MotionActivityEvent>()
  for (const event of activities.value) {
    if (!rows.has(event.camera_id)) rows.set(event.camera_id, event)
  }
  return rows
})
...
function activityRelative(value: string) {
  ...
  if (!Number.isFinite(parsed)) return activityClock(value)
...
function writePlaybackAutostart(eventId: number) {
  try {
    ...
    // Playback still opens passively if session storage is unavailable.
  }
}
...
function savedWall(): SavedWall {
  try {
    const raw = window.localStorage.getItem(LIVE_WALL_STORAGE_KEY)
    if (!raw) return {}
    const value = JSON.parse(raw) as SavedWall
...
function primeLiveCamera(cameraId: number) {
  try {
    ...
    // Live falls back to its existing saved/default layout.
  }
...
function openHealth() {
  void router.push('/health-center')
}
...
<template>
  <div class="protect-home" v-loading="loading">
    <header class="home-header">
      ...
      <div class="health-pill" :class="overallHealthy ? 'healthy' : 'attention" role="button" tabindex="0" @click="openHealth" @keydown.enter="openHealth">
    ...
    <section class="status-strip" aria-label="系统状态摘要">
      <div class="status-cell">
        <span>摄像头</span>
        <strong>{{ summary?.cameras.online ?? '-' }}<small> / {{ summary?.cameras.enabled ?? '-' }} 在线</small></strong>
      </div>
      <div class="status-cell">
        <span>录像</span>
        <strong>{{ summary?.cameras.recording ?? '-' }}<small> 路运行中</small></strong>
      </div>
      <div class="status-cell">
        <span>存储</span>
        <strong>{{ summary?.storage.used_percent ?? '-' }}<small>% 已使用</small></strong>
      </div>
      <div class="status-cell" :class="{ attention: (summary?.cameras.abnormal || 0) > 0 || uploadFailures > 0 }">
        <span>需要关注</span>
        <strong>{{ (summary?.cameras.abnormal || 0) + uploadFailures }}<small> 项</small></strong>
      </div>
    </section>
    ...
    <section class="home-primary-grid">
      <article class="surface activity-surface">
        ...
        <div v-else-if="!recentActivities.length && !activityLoading" class="activity-empty">
          <span class="empty-motion-mark"></span>
          <strong>今天还没有检测活动</strong>
          <span>新的移动事件会显示在这里，不会自动打开任何视频流。</span>
        </div>
      ...
      <aside class="surface system-surface">
        <header class="section-head compact">
          ...
          <button type="button" class="quiet-link" @click="openHealth">详情 ›</button>
        </header>
        ...
        <div class="system-body">
          <div class="storage-line">
            <div>
              <span>录像存储</span>
              <strong>{{ summary?.storage.used_percent ?? '-' }}%</strong>
            </div>
          ...
          <div class="system-list">
            <div>
              <span>24 小时录像</span>
              <strong>{{ summary?.recordings_24h.segments ?? '-' }} 片段</strong>
              <small :class="{ bad: (summary?.recordings_24h.unhealthy_segments || 0) > 0 }">{{ summary?.recordings_24h.unhealthy_segments ?? 0 }} 异常</small>
            </div>
            <div>
              <span>上传队列</span>
              <strong>{{ pendingUploads }} 待处理</strong>
              <small :class="{ bad: uploadFailures > 0 }">{{ uploadFailures }} 失败</small>
            </div>
            <div>
              <span>FFmpeg / setts</span>
              <strong>{{ system?.ffmpeg?.setts_available === false ? '异常' : system?.ffmpeg ? '正常' : '检测中' }}</strong>
              <small>{{ system?.ffmpeg?.ffmpeg_version || '运行环境' }}</small>
            </div>
            <div>
              <span>OpenList 上传</span>
              <strong>{{ system?.upload?.enabled ? (system?.upload?.configured ? '已连接' : '未配置') : '未启用' }}</strong>
              <small>{{ system?.upload?.active ? '正在上传' : '当前空闲' }}</small>
            </div>
          </div>
        </div>
      </aside>
    ...
    <section class="surface camera-surface">
      <header class="section-head camera-head">
        <div>
          <span class="section-kicker">CAMERAS</span>
          <h2>摄像头状态</h2>
          <p>这里只显示运行状态和最近活动；进入 Live 后仍需手动开始画面。</p>
        </div>
        <span class="camera-total">{{ summary?.cameras.enabled ?? 0 }} 台已启用</span>
      </header>

      <div class="camera-status-grid">
        <article v-for="camera in summary?.camera_health || []" :key="camera.camera_id" class="camera-row" :class="{ abnormal: camera.abnormal }">
          <button type="button" class="camera-main" @click="primeLiveCamera(camera.camera_id)">
            ...
            <span class="camera-identity">
              <strong>{{ camera.name }}</strong>
              <small>{{ camera.ip }}</small>
            </span>
            <span class="camera-facts">
              <span :class="connectivityClass(camera)">{{ connectivityLabel(camera.connectivity_status) }}</span>
              <span :class="recorderClass(camera.recorder_state)">{{ recorderLabel(camera.recorder_state) }}</span>
            </span>
            <span class="camera-activity">
              <small>最近活动</small>
              <strong>{{ latestActivityLabel(camera.camera_id) }}</strong>
            </span>
...
.home-header { display: flex; align-items: flex-end; justify-content: space-between; gap: 24px; margin-bottom: 18px; }
.home-heading { min-width: 0; }.eyebrow,.section-kicker { display: block; color: #647387; font-size: 9px; font-weight: 800; letter-spacing: .18em; }
.home-heading h1 { margin: 5px 0 4px; font-size: 25px; line-height: 1.2; font-weight: 680; letter-spacing: -.025em; }.home-heading p,.section-head p { margin: 0; color: var(--nvr-muted); font-size: 11px; }
...
@media (max-width: 980px) { .home-primary-grid { grid-template-columns: 1fr; }.system-surface { order: 2; }.system-list { display: grid; grid-template-columns: 1fr 1fr; gap: 0 18px; }.camera-status-grid { grid-template-columns: 1fr; } }
@media (max-width: 720px) { .protect-home { padding: 15px; }.home-header { align-items: flex-start; flex-direction: column; }.health-pill { width: 100%; }.status-strip { grid-template-columns: repeat(2,minmax(0,1fr)); }.status-cell:nth-child(2) { border-right: 0; }.status-cell:nth-child(-n+2) { border-bottom: 1px solid var(--nvr-border); }.activity-grid { grid-template-columns: 1fr; }.section-head { align-items: flex-start; }.camera-main { grid-template-columns: 8px minmax(90px,1fr) auto; }.camera-facts { justify-content: flex-end; }.live-link { display: none; }.home-heading h1 { font-size: 22px; } }
</style>
