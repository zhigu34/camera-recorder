...
const storageUsedBytes = computed(() => Math.max(
  0,
  Number(summary.value?.storage.used_bytes ?? (
    Number(summary.value?.storage.total_bytes || 0) - Number(summary.value?.storage.free_bytes || 0)
  )),
))
const localRecordingBytes = computed(() => Math.max(0, Number(summary.value?.storage.local_recordings_bytes || 0)))
...
<template>
  <div class="protect-home" v-loading="loading">
    ...
    <section class="status-strip" aria-label="系统状态摘要">
      ...
      <div class="status-cell"><span>磁盘</span><strong>{{ summary?.storage.used_percent ?? '-' }}<small>% 已使用</small></strong></div>
      ...
      <aside class="surface system-surface">
        ...
        <div class="system-body">
          <div class="storage-line">
            <div class="storage-title"><span>本地录像占用</span><strong>{{ bytes(localRecordingBytes) }}</strong></div>
            <div class="storage-track"><i :style="{ width: `${Math.min(100, summary?.storage.used_percent || 0)}%` }"></i></div>
            <div class="storage-copy"><span>磁盘 {{ summary?.storage.used_percent ?? '-' }}% · {{ bytes(storageUsedBytes) }} 已用</span><span>{{ bytes(summary?.storage.free_bytes) }} 可用</span></div>
          </div>
