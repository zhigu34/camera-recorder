...
function savePreferences() {
  try {
    window.localStorage.setItem(CAMERA_PREFERENCES_KEY, JSON.stringify({ filter: filter.value, sortKey: sortKey.value }))
  } catch { /* browser storage restrictions must not block management */ }
}
...
async function loadData(showLoading = true, force = true) {
  ...
  try {
    ...
    if (requestedId !== null) syncDrawerFromLocation(showLoading)
    else if (selectedCamera.value) {
      selectedCamera.value = cameraById(selectedCamera.value.id) || null
    ...
    if (showLoading) localLoading.value = false
  }
}
function resetForm() {
...
async function saveCamera() {
  ...
  try {
    ...
    if (editingId.value === null) { await axios.post('/api/cameras', payload); ElMessage.success('摄像头已添加') }
    else { await axios.put(`/api/cameras/${editingId.value}`, payload); ElMessage.success('摄像头配置已保存') }
...
async function removeCamera(camera: Camera) {
  ...
  try {
    ...
    if (selectedCamera.value?.id === camera.id) { previewPlaying.value = false; drawerVisible.value = false; writeCameraDeepLink(null, 'replace') }
    cameraStore.invalidate()
    await loadData(false, true)
  } catch (error) {
...
watch(() => route.query.camera_id, () => syncDrawerFromLocation(false))
onMounted(() => {
  loadPreferences()
...
<template>
  <section class="camera-page" v-loading="loading">
    ...
    <div class="summary-grid">
      <button class="summary-card" :class="{ active: filter === 'all' }" @click="setFilter('all')"><span>全部摄像头</span><strong>{{ summary.total }}</strong><small>已配置设备</small></button>
      <button class="summary-card online" :class="{ active: filter === 'online' }" @click="setFilter('online')"><span>在线</span><strong>{{ summary.online }}</strong><small>最近连接正常</small></button>
      <button class="summary-card issue" :class="{ active: filter === 'issue' }" @click="setFilter('issue')"><span>异常 / 未检测</span><strong>{{ summary.issue }}</strong><small>建议执行连接检测</small></button>
      <button class="summary-card recording" :class="{ active: filter === 'recording' }" @click="setFilter('recording')"><span>录像中</span><strong>{{ summary.recording }}</strong><small>当前录像进程</small></button>
    </div>
    <div class="toolbar">
      <el-input v-model="query" clearable :prefix-icon="Search" placeholder="搜索名称、厂商、型号、IP 或 RTSP 路径" class="search-box" />
      <div class="camera-toolbar-actions"><el-select v-model="sortKey" class="camera-sort-select" aria-label="设备排序" title="设备排序" @change="savePreferences"><el-option v-for="item in sortOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select><el-button v-if="issueCameras.length || batchProbeRunning" class="batch-probe-button" :icon="Connection" :loading="batchProbeRunning" :disabled="actionCameraId !== null && !batchProbeRunning" @click="runBatchProbe">{{ batchProbeLabel }}</el-button></div>
      <span class="result-count">显示 {{ filteredCameras.length }} / {{ cameras.length }} 台</span>
    </div>
    <div v-if="filteredCameras.length" class="camera-list">
      <article v-for="camera in filteredCameras" :key="camera.id" class="camera-card" :class="{ selected: drawerVisible && selectedCamera?.id === camera.id, 'needs-probe': canQuickProbe(camera), busy: isCameraBusy(camera.id) }" role="button" tabindex="0" :aria-label="`查看 ${camera.name} 详情`" @click="openDetails(camera)" @keydown.enter.prevent="openDetails(camera)" @keydown.space.prevent="openDetails(camera)">
        ...
        <div class="camera-copy"><div class="camera-name-row"><strong :title="camera.name">{{ camera.name }}</strong><span class="health-badge" :class="health(camera)"><i></i>{{ healthLabel(camera) }}</span><span class="record-badge" :class="{ active: isRecording(camera.id) }"><i></i>{{ runtimeLabel(camera) }}</span></div><div class="camera-identity" :title="identitySummary(camera)">{{ identitySummary(camera) }}</div><div class="camera-video" :title="videoSummary(camera)">{{ videoSummary(camera) }}</div><div class="camera-address" :title="`${camera.ip}:${camera.rtsp_port} · ${camera.rtsp_path}`">{{ camera.ip }}:{{ camera.rtsp_port }} · {{ camera.rtsp_path }}</div></div>
    ...
    <el-drawer v-model="drawerVisible" size="680px" class="camera-detail-drawer" @closed="closeDrawer">
      <template #header><div v-if="selectedCamera" class="drawer-title"><div class="drawer-title-copy"><strong :title="selectedCamera.name">{{ selectedCamera.name }}</strong><span :title="identitySummary(selectedCamera)">{{ identitySummary(selectedCamera) }}</span></div><div class="drawer-title-states"><span class="health-badge" :class="health(selectedCamera)"><i></i>{{ healthLabel(selectedCamera) }}</span><span class="record-badge" :class="{ active: isRecording(selectedCamera.id) }"><i></i>{{ runtimeLabel(selectedCamera) }}</span></div></div></template>
      <div v-if="selectedCamera" class="drawer-body drawer-body-v2">
        <nav v-if="navigationCameras.length > 1" class="drawer-device-nav" aria-label="摄像头切换"><el-button text :icon="ArrowLeft" :disabled="!previousCamera || batchProbeRunning || actionCameraId !== null" :title="previousCamera ? `上一台：${previousCamera.name}` : '已经是第一台'" @click="switchDetails(previousCamera)">上一台</el-button><div class="drawer-device-nav-position"><strong>{{ selectedNavigationIndex + 1 }} / {{ navigationCameras.length }}</strong><span>{{ navigationScopeLabel }} · ← →</span></div><el-button text :icon="ArrowRight" :disabled="!nextCamera || batchProbeRunning || actionCameraId !== null" :title="nextCamera ? `下一台：${nextCamera.name}` : '已经是最后一台'" @click="switchDetails(nextCamera)">下一台</el-button></nav>
        <section class="device-overview"><div class="device-overview-visual" :class="health(selectedCamera)"><CameraDeviceGlyph :form-factor="selectedCamera.form_factor" /></div><div class="device-overview-main"><div class="device-overview-heading"><div><strong :title="selectedCamera.manufacturer || '通用 RTSP 摄像头'">{{ selectedCamera.manufacturer || '通用 RTSP 摄像头' }}</strong><span :title="`${selectedCamera.model || formFactorLabel(selectedCamera.form_factor)} · #${selectedCamera.id}`">{{ selectedCamera.model || formFactorLabel(selectedCamera.form_factor) }} · #{{ selectedCamera.id }}</span></div><span class="device-overview-type">{{ formFactorLabel(selectedCamera.form_factor) }}</span></div><dl class="device-overview-facts"><div><dt>地址</dt><dd :title="`${selectedCamera.ip}:${selectedCamera.rtsp_port}`">{{ selectedCamera.ip }}:{{ selectedCamera.rtsp_port }}</dd></div><div><dt>视频</dt><dd :title="videoSummary(selectedCamera)">{{ videoSummary(selectedCamera) }}</dd></div><div><dt>最近在线</dt><dd>{{ formatTime(selectedCamera.last_online_at) }}</dd></div></dl></div></section>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId === null ? '添加 RTSP 摄像头' : '编辑摄像头'" width="680px" destroy-on-close>
      <el-form label-width="88px" class="camera-form" @submit.prevent="saveCamera"><div class="form-grid"><el-form-item label="名称" class="wide"><el-input v-model="form.name" placeholder="例如：门口摄像头" /></el-form-item><el-form-item label="厂商"><el-input v-model="form.manufacturer" placeholder="例如 Hikvision" /></el-form-item><el-form-item label="型号"><el-input v-model="form.model" placeholder="例如 DS-2CD..." /></el-form-item><el-form-item label="外形" class="wide"><el-select v-model="form.form_factor" style="width: 100%"><el-option v-for="item in formFactorOptions" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item><el-form-item label="IP 地址"><el-input v-model="form.ip" placeholder="192.168.1.101" /></el-form-item><el-form-item label="RTSP 端口"><el-input-number v-model="form.rtsp_port" :min="1" :max="65535" controls-position="right" /></el-form-item><el-form-item label="用户名"><el-input v-model="form.username" /></el-form-item><el-form-item label="密码"><el-input v-model="form.password" type="password" show-password :placeholder="editingId === null ? '必填' : '留空保持原密码'" /></el-form-item><el-form-item label="主码流" class="wide"><el-input v-model="form.rtsp_path" placeholder="/ch1/main" /></el-form-item><el-form-item label="子码流" class="wide"><el-input v-model="form.sub_rtsp_path" placeholder="可选，例如 /ch1/sub" /></el-form-item><el-form-item label="时间戳"><el-select v-model="form.timestamp_mode" style="width: 100%"><el-option label="重建时间戳（推荐）" value="reconstruct" /><el-option label="使用原始时间戳" value="native" /><el-option label="使用系统时间" value="wallclock" /></el-select></el-form-item><div class="switch-group"><label><span>启用摄像头</span><el-switch v-model="form.enabled" /></label><label><span>自动录像</span><el-switch v-model="form.auto_record" /></label></div></div></el-form>
